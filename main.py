...
class AddTeamModal(discord.ui.Modal, title="Add New Team"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = make_embed(
            "Team Created",
            f"Team **{str(self.club_name).strip()}** has been successfully added to the database.",
            discord.Color.teal(),
...
class RemoveTeamSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not club:
            await interaction.response.send_message("Team not found in the database.", ephemeral=True)
            return
        ...
        embed = make_embed(
            "Team Removed",
            f"Team **{team_name}** has been permanently deleted from the database.",
            discord.Color.red(),
            "Administrator Action"
        )
...
class ManagerUserSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not isinstance(target_user, discord.Member):
            await interaction.response.send_message("Please select a valid server member.", ephemeral=True)
            return

        data = load_clubs()
        if not data["clubs"]:
            await interaction.response.send_message("Please create at least one team first using the Add Team button.", ephemeral=True)
            return
...
class ContractView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not club:
            await interaction.response.send_message("The team associated with this contract no longer exists.", ephemeral=True)
            return

        if self.manager_id not in club.get("managers", []):
            await interaction.response.send_message("The manager who sent this contract no longer has permissions for this team.", ephemeral=True)
            return

        if len(club.get("players", [])) >= MAX_PLAYERS_PER_CLUB:
            await interaction.response.send_message(f"This team has reached the maximum capacity of {MAX_PLAYERS_PER_CLUB} players.", ephemeral=True)
            return

        old_club_key, old_club = find_player_club(self.player_id)
        if old_club_key and old_club_key != self.club_key:
            await interaction.response.send_message(
                f"You are already signed to **{old_club.get('name', old_club_key)}**. You must leave your current team before joining another.",
                ephemeral=True
            )
            return
...
@bot.tree.command(name="panel", description="Open the administrator team management panel")
async def panel(interaction: discord.Interaction):
    ...
    embed.add_field(name="Remove Team", value="Permanently deletes an existing team from the database.", inline=False)
...
async def contract(interaction: discord.Interaction, user: discord.Member):
    ...
    if len(players) >= MAX_PLAYERS_PER_CLUB:
        await interaction.response.send_message(
            f"**{club['name']}** has reached the maximum roster size of {MAX_PLAYERS_PER_CLUB} players.",
            ephemeral=True
        )
        return
    ...
    if current_club_key:
        await interaction.response.send_message(
            f"{user.mention} is already signed to **{current_club.get('name', current_club_key)}**. A player cannot be signed to multiple teams.",
            ephemeral=True
        )
        return
...
@bot.tree.command(name="releaseplayer", description="Remove a player from your team roster")
...
async def releaseplayer(interaction: discord.Interaction, user: discord.Member):
    ...
    if user.id not in players:
        await interaction.response.send_message(f"{user.mention} is not currently on the roster of **{club['name']}**.", ephemeral=True)
        return
...
async def roster(interaction: discord.Interaction, club: str):
    ...
    if not players:
        ...
        for player_id in players:
            ...
            lines.append(member.name if member else f"User-{player_id}")
        roster_text = "\n".join(lines)
