...
class RemoveTeamSelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(
            placeholder="Select the team to remove",
            min_values=1,
            max_values=1,
            options=options[:25],
...
async def painel(interaction: discord.Interaction):
    ...
    embed = make_embed(
        ...
        "Use the buttons below to create teams, remove teams, add managers, or remove managers.",
        discord.Color.dark_teal(),
        "Administrator only"
    )
    embed.add_field(name="Add New Team", value="Create a new team in the database.", inline=False)
    embed.add_field(name="Remove Team", value="Delete an existing team permanently.", inline=False)
