...
CANAL_ACCEPT_ID = 1512973313897660436
CANAL_RELEASE_ID = 1512973313897660436
CANAL_FREEAGENCY_ID = 1512973228933517342

CLUBS_FILE = "clubs.json"
MAX_PLAYERS_PER_CLUB = 14
...
class ContractView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not club:
            await interaction.followup.send("The team associated with this contract no longer exists.", ephemeral=True)
            return

        if self.manager_id not in club.get("managers", []):
            await interaction.followup.send("The manager who sent this contract no longer has permissions for this team.", ephemeral=True)
            return

        if len(club.get("players", [])) >= MAX_PLAYERS_PER_CLUB:
            await interaction.followup.send(f"This team has reached the maximum capacity of {MAX_PLAYERS_PER_CLUB} players.", ephemeral=True)
            return
...
async def roster(interaction: discord.Interaction, club: str):
    ...
    embed = discord.Embed(
        title=f"Roster of {selected_club['name']}",
